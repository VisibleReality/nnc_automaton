...
def get_random_image_from_web (output_location: str) -> None:
	...
	page_info: dict[str, any] = requests.get \
		("https://www.besthdwallpaper.com/anime-wallpapers-ct_en-US-51/1?isJson=true").json()["pagingInfo"]
	...
	while not got_image:
		...
		page_data: list[dict[str, any]] = requests.get \
			(f"https://www.besthdwallpaper.com/anime-wallpapers-ct_en-US-51/{page_number}?isJson=true").json()["data"]
